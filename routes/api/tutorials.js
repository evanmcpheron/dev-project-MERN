...
router.post('/', async (req, res) => {
  const { title, description, thumbnailURL, status, date } = req.body;

  try {
    let tutorial = await Tutorial.findOne({ title });

    if (tutorial) {
      return res.status(400).json({ errors: [{ msg: 'Tutorial name already exists' }] });
    }
...
// @route    POST `:id
// @desc     Update a tutorial series
// @access   Admin
router.post('/update/:id', auth, async (req, res) => {
  try {
    let tutorial = await Tutorial.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    const tutorials = await Tutorial.find().populate('tutorial', [
      'title',
...
// @access   Private
router.post(
  '/comment/update/:id/:video_id/:comment_id',
  [auth, [check('text', 'Text is required').not().isEmpty()]],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      // Pull out comment
      const video = tutorial.video.find((video) => video.id === req.params.video_id);

      for (let i = 0; i < tutorial.video.length; i++) {
        if (tutorial.video[i]._id == req.params.video_id) {
          for (let e = 0; e < video.comments.length; e++) {
            if (video.comments[e]._id == req.params.comment_id) {
              const editedComment = {
                text: req.body.text,
                name: user.fName + ' ' + user.lName,
                avatar: user.avatar,
              ...
              video.comments.splice(e, 1, editedComment);
              await tutorial.save();
...
router.delete('/comment/:id/:video_id/:comment_id', auth, async (req, res) => {
  try {
    ...
    const comment = video.comments.find((comment) => comment.id === req.params.comment_id);
